	...
	https.get(addr_call,function(res){
		...
		res.on('end', function(){
			try{
				...
				if(data.status !== 1){
					callback("Error in the status of the balance", null);
				}else{
					callback(err, data);
				}
			}catch(e){
		...
		for(var i = 0; i < list.length; i++){
			(function(l, index, acc){
				...
				getBalanceByAddr(addr, function(err, data){
					...
					/*
					* Set balance to 0 if no data found (no transaction, brand new address)
					*/
					var balanceTmp = 0;
					if(data.data && data.data[0] && data.data[0].balance){
						balanceTmp = data.data[0].balance;
					}else{
						console.log("Empty balance or malformed object in getBalanceByUser : ");
						balanceTmp = 0;
					}

					acc.data.push({"addr" : addr, "balance" : balanceTmp});
					done++;
...
var _getPrice = function(callback){
	...
	var req = https.get("https://min-api.cryptocompare.com/data/price?fsym=ETH&tsyms=USD,EUR", function (res) {
		var chunks = [];
