...
var ethToCurrency = function(eth, currency){
	var value = 0;
	var price = robot.brain.get("ether_price")[0];

	if(price.hasOwnProperty("price_" + currencyToA[currency])){
		currencyMult = price["price_" + currencyToA[currency]];
	}else if(price.hasOwnProperty("price_" + currencyToA[currency])){
		currencyMult = price["price_" + currencyToA[currency]];
	}else{
...
var currencyToEth = function(value, currency){
	var result = 0;
	var price = robot.brain.get("ether_price");

	if(price.hasOwnProperty("price_" + currency)){
		currencyMult = robot.brain.get("ether_price").price["price_" + currency];
	}else if(price.hasOwnProperty(currencyToA["price_" + currency])){
		currencyMult = robot.brain.get("ether_price").price[currencyToA["price_" + currency]];
	}else{
		throw "Currency not found";
	}
